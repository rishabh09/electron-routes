...
function enhanceUploadData(uploadData: UploadData[]): EnhancedUploadData[] {
  return uploadData.map((data: UploadData) => {
    const enhancedData: EnhancedUploadData = {
      ...data,
      stringContent: null,
      json: null
    };
    if (data.bytes && data.bytes.toString) {
      ...
      enhancedData.json = (): AnyJson =>
        enhancedData.stringContent && JSON.parse(enhancedData.stringContent());
    }
    return enhancedData;
  });
}
  ...
  private _handle(
    ...
    if (handlers.length === 0) {
      ...
      const req: ElectronRequest = {
        ...
        body:
          uploadData_.length && uploadData_[0].json
            ? uploadData_[0].json()
            : {},
        url: path,
        headers,
